...
def lyrics(bot: Bot, update: Update, args: List[str]):
    ...
    reply_text = f'Musiqinin Sözlərini Axtarır...'
    ...
    if len(song) == 2:
        ...
        try:
            ...
            return update.effective_message.reply_text("Musiqi %s Tapılmadı :(" % song[1], failed=True)
        else:
            lyricstext = LYRICSINFO % (song[0].replace(
                " ", "_"), song[1].replace(" ", "_"))
            return update.effective_message.reply_text(lyrics + lyricstext, parse_mode="MARKDOWN")
    else:
        return update.effective_message.reply_text("Hayda Xəta Diyəsən bu növdə əmr verməlisən İFAÇI+MUSİQİ ADI:)"), failed=True)


__help__ = """
 - /lyrics <musiqi> Favori Musiqinizi Yazaraq Həmin Musiqinin Sözlərini Tapa Bilərsininz!
 - Qeyd:Azərbaycan Musiqilərə Adətən Tapılmır!
"""
