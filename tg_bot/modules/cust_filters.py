...
HANDLER_GROUP = 10
BASIC_FILTER_STRING = "*Söhbətdəki Filtrlər:*\n"
...
def list_handlers(bot: Bot, update: Update):
    ...
    if not all_handlers:
        update.effective_message.reply_text("{} qrupundan Filtr yoxdur!".format(chat_name))
        return
...
def filters(bot: Bot, update: Update):
    ...
    if len(extracted) >= 2:
        ...
        if not content:
            msg.reply_text("Qeyd mesajı yoxdur - İndi button əlavə edə bilmərsən, bununla biryerdə mesaj verməlisən!")
            return
    ...
    msg.reply_text("`{}` filtri *{}* qrupunda əlavə edildi !".format(keyword, chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
...
def stop_filter(bot: Bot, update: Update):
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            update.effective_message.reply_text("Yep, *{}* qrupunda bu mesaja cavab vermiyəcəm .".format(chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
            raise DispatcherHandlerStop
            ...
            if filt.is_sticker:
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("Deyəsən dəstəklənməyən url protokolundan istifadə etməyə çalışırsınız. "
                                           "məsələn tg:// kimi. Zəhmət olmasa bir daha sınayın :/")
...
def __stats__():
    return "{} filtr mövcuddur, ümumi {} qrupda.".format(sql.num_filters(), sql.num_chats())
...
__help__ = """
 - /filters: qrupdakı aktiv filtrləri göstərir.

*Admin only:*
 - /filter <açar söz> <cavab veriləcək söz>: qrupa bir filtr əlavə edər. Bu açar söz yazılanda bot avtomatik sizin seçdiyiniz cavabı verər.\
Əgər siz stikerə yanıt verib `/filter <açar söz>` yazsanız kimsə açar sözü desə bot ona avtomstik bu stikeri atacaq, QEYD: Əgər \
söz yox cümləni filter etmək istəyirsinizsə dırnaq işarəsindən istifadə edin. Məsələn: /filter "salam aleykum" aleykum salam \
necəsən?
 - /stop <filtr açar sözü>: verilən filtri silər.
"""
