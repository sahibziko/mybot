...
CURRENT_WARNING_FILTER_STRING = "<b>Qrupdakı hazırki xəbərdarlıq filtrləri:</b>\n"


# Not async
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("pf adminlərə xəbərdarlıq verə bilmərəm!")
        return ""
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} xəbərdarlıq limitinə çatdı, {} qrupdan atıldı!".format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "{} xəbərdarlıq limitinə çatdı, {} banlandı!".format(limit, mention_html(user.id, user.first_name))
        ...
        reply = "{} istifadəçisinin {}/{} xəbərdarlığı var... diqqətli ol limitə çatmasın!".format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
            reply += "\nSonuncu xəbərdarlığın səbəbi:\n{}".format(html.escape(reason))
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "İstifadəçinin xəbərdarlığı yoxdu.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def warn_user(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("Heç bir istifadəçi təyin edilmədi")
    return ""
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "Bu istifadəçinin {}/{} xəbərdarlığı var, səbəblər aşağıda verilib:".format(num_warns, limit)
            ...
            update.effective_message.reply_text(
                "Bu istifadəçinin {}/{} xəbərdarlığı var, amma səbəb verilməyib.".format(num_warns, limit))
    else:
        update.effective_message.reply_text("Bu istifadəçinin xəbərdarlığı yoxdu!")
...
def add_warn_filter(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text("'{}' üçün xəbərdaredici əlavə edildi!".format(keyword))
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("Yep, Artıq buna görə insanlara xəbərdarlıq verməyəcəm.")
            raise DispatcherHandlerStop

    msg.reply_text("Bu aktiv xəbərdarlıq filtri deyil - /warnlist yazaraq aktiv xəbərdarlıq filtrlərinə baxa bilərsən.")
...
def list_warn_filters(bot: Bot, update: Update):
    ...
    if not all_handlers:
        update.effective_message.reply_text("Burada aktiv xəbərdarlıq filtri yoxdur!")
        return
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                msg.reply_text("Yeni xəbərdarlıq limiti təyin olundu. Yeni limit --> {}".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#SET_WARN_LIMIT" \
                       "\n<b>Admin:</b> {}" \
            ...
            msg.reply_text("Mənə bir rəqəm verməlisən!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text("Artıq xəbərdarlıq limitinə çatanlar qrupdan atılacaq! Amma sonradan qoşula bilər.")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
...
def __stats__():
    return "{} ədəd xəbərdarlıq, ümumi {} ədəd qrupda.\n" \
           "{} ədəd xəbərdarlıq filtri, ümumi {} ədəd qrupda.".format(sql.num_warns(), sql.num_warn_chats(),
...
def __chat_settings__(chat_id, user_id):
    ...
    return "Bu qrupda `{}` ədəd xəbərdarlıq filtri var. Limit `{}` ədəd xəbərdarlıqdır " \
           "əgər istifadəçi limiti keçsə *{}*.".format(num_warn_filters, limit, "qrupdan atılacaq" if soft_warn else "banlanacaq")


__help__ = """
 - /warns <istifafəçi>: itifadəçinin xəbərdarlıqlarını göstərir.
 - /warnlist: xəbərdarlıq fitrlərinin siyahısı

*Sadəcə adminlər:*
 - /warn <istifadəçilər>: istifadəçiyə xəbərdarlıq verir. 3 xəbərdarlıqdan sonra, istifadəçi qrupdan banlanır.
 - /resetwarn <istifadəçi>: istifadəçinin xəbərdarlıqlarını sıfırlayır.
 - /addwarn <söz> <cavab>: xəbərdarlıq filtri əlavə edir. Söz yerinə cümlə də yazmaq olar \
bunun üçün dırnaqlardan istifadə etmək lazımdır, məsələn: `/addwarn "mal adminlər" Adminlərinlə düzgün danış mal user.`. 
 - /nowarn <söz>: xəbərdarlıq filtrini dayandırır
 - /warnlimit <rəqəm>: xəbərdarlıq limitini təyin edir
 - /strongwarn <on/yes/off/no>: əgər on və ya yes təyin etsəniz xəbərdarlıq limitinə çatanlar banlanacaq. off və ya no təyin etsəniz xəbərdarlıq limitinə çatanlar qrupdan atılacaq
"""
