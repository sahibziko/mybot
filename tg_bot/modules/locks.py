    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("{} kilidi açıldı!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                ...
                message.reply_text("Nəyin kilidi açmağa çalışırsan...? /locktypes kilidlənə bilən şey şüylərə baxa bilərsən")

        else:
            bot.sendMessage(chat.id, "Nəyin kilidini açmağa çalışırsan...?")
...
__help__ = """
 - /locktypes: kilidlənə bilən şey şüylərin siyahısı

*Sadəcə adminlər:*
 - /lock <növ>: müəəyyən şeyi kilidləyir (şəxsidə mövcud deyil)
 - /unlock <növ>: müəəyyən şeyi kilidləyir (şəxsidə mövcud deyil)
 - /locks: kilidlənə bilən şey şüylərin siyahısı.

Kilidlər qrup icazələrini dəyişir.
məsələn:
Url kilidlənəndə içində url olan hər bir mesaj avtomatik silinir, stikerləri kilidləndə \
kimsə stiker atsa avtomatik silinəcəy və s.
Botları kilidləyəndə adminlər xarici heç kim qrupa not ata bilməyəcək.
"""
