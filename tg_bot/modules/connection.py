    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Bu söhbətlə əlaqələr istifadəçilər üçün deaktiv edildi.")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Bu söhbətlə əlaqələr istifadəçilər üçün aktiv edildi")
            else:
                update.effective_message.reply_text("Zəhmət olmasa on/yes/off/no daxil edin!")
        else:
            update.effective_message.reply_text("Zəhmət olmasa on/yes/off/no daxil edin!")
    else:
        update.effective_message.reply_text("Zəhmət olmasa on/yes/off/no daxil edin!")
        ...
        if len(args) >= 1:
            ...
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("*{}* söhbətinə uğurla qoşuldu".format(chat_name), parse_mode=ParseMode.MARKDOWN)
                ...
                update.effective_message.reply_text("Bu söhbətıə əlaqələr deaktiv edilib!")
        else:
            update.effective_message.reply_text("Əlaqə yaratmaq üçün Chat İD verin!")
            history = sql.get_history(user.id)
            print(history.user_id, history.chat_id1, history.chat_id2, history.chat_id3, history.updated)
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
            sql.disconnected_chat = update.effective_message.reply_text("Söhbətlə əlaqə uğurla dayandırıldı!")
            #Rebuild user's keyboard
            keyboard(bot, update)
        else:
           update.effective_message.reply_text("Söhbətlə əlaqəni dayandırmaq uğursuz oldu!")
    else:
        update.effective_message.reply_text("İstifadə yalnız PM-lərlə məhdudlaşır")
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("Qoşulacaq söhbətdə admin olmalısan!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("Qrup əlaqələrlə bağlı səlahiyyətləri dəyişib və ya sən orada admin deyilsən.\nSəni Disconnect edəcəm.")
...
__help__ = """
Qoşulu qruplarla mövcud əməliyyatlar:
 • Qeydləri görməy və dəyişmək
 • Filterləri görmək və dəyişmək

 - /connect <chatid>: Söhbətlə əlaqə yaradır
 - /disconnect: Söhbətlə əlaqəni kəsir
 - /allowconnect on/yes/off/no: Əlaqələri açmaq bağlamaq
"""
