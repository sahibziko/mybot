...
RUN_STRINGS = (
    "Hara qaçırsan?",
    "Huh? nə? onlar qaçdılar?",
    "ZZzzZZzz... Huh? nə? oh, yenə onlar, nevermind.",
    "Tez qayıt bura!",
    "Elə də sürətli deyil...",
    "Məni onlarla tək buraxma!!",
    "Qaçsan ölərsən.",
    "Zarafat eləyirəm, mən hər yerdəyəm",
    "Bundan peşman olacaqsan...",
    "Sən /kickme də istifadə edə bilərsən, eşitdiyimə görə bu əyləncəlidir.",
    "Get başqasını narahat elə, urada heçkim vecinə almır.",
    "Qaça bilərsən amma gizlənə bilmərsən.",
    "Edə biləcəyin hər şey budur?",
    "Arxandayam...",
    "You've got company!",
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id:
        ...
        msg.reply_text("Bir xətaya yol verdin.")
        return

    else:
        return

    text = "<b>İstifadəçi haqqında məlumat</b>:" \
           "\nID: <code>{}</code>" \
           "\nAd: {}".format(user.id, html.escape(user.first_name))

    if user.last_name:
        text += "\nSoyad: {}".format(html.escape(user.last_name))

    if user.username:
        text += "\nİstifadəçi adı: @{}".format(html.escape(user.username))
        ...
        if user.id in SUDO_USERS:
            ...
            if user.id in SUPPORT_USERS:
                text += "\nBu şəxs mənim support istifadəçilərimdən biridir! " \
                        "Sudo qədər güclü deyil amma gban ata bilər."

            if user.id in WHITELIST_USERS:
                text += "\nBu istifadəçi ağ siyahıdadır! " \
                        "Yəni mən ona ban/mute/kick verə bilmərəm."
