...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("`{}` əmri deaktiv edildi".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Bu əmr deaktiv edilə bilməz")

        else:
            update.effective_message.reply_text("Deaktiv ediləcək əmri vermədinsə nəyi deaktiv edə bilərəmki?")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("`{}` əmrinin istifadəsi aktiv edildi".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Bu heç deaktiv olmuşduki??")

        else:
            update.effective_message.reply_text("Aktiv ediləcək əmri vermədinsə nəyi aktiv edə bilərəmki?")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("Aşağıdakı əmrlər aktiv/deaktiv edilə bilər bilər:\n{}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Heç bir əmr deaktiv edilə bilməz!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Aşağıdakı əmrlər hazırda deaktiv edilib:\n{}".format(result)
    ...
    def __stats__():
        return "{} ədəd əmr deaktiv edilib, ümumi {} ədəd qrupda.".format(sql.num_disabled(), sql.num_chats())
    ...
    __mod_name__ = "Əmrləri deaktiv etmək"

    __help__ = """
 - /cmds: deaktiv edilən əmrləri göstərir

*Sadəcə adminlər:*
 - /enable <əmr adı>: əmri aktiv edir
 - /disable <cmd name>: əmri deaktiv edir
 - /listcmds: aktiv/deaktiv edilə bilən əmrlərin siyahısını verir
    """
